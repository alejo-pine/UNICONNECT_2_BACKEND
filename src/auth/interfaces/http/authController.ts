...
const resolveNeedsOnboarding = async (profileId: string, created: boolean): Promise<boolean> => {
  if (created) {
    const onboardingCreationResult =
      await onboardingDependencies.markNewProfileOnboardingRequiredUseCase.execute(profileId);
  ...
  const onboardingStatusResult = await onboardingDependencies.getOnboardingStatusUseCase.execute(profileId);
